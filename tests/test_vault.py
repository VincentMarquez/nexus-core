...
def test_present_from_env(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setenv("OPENAI_API_KEY", "sk-FAKE-TEST-VALUE-NOT-A-CREDENTIAL")
    v = Vault(workdir=tmp_path)
    assert v.present("OPENAI_API_KEY") is True
    assert v.source_of("OPENAI_API_KEY") == "env"
    assert v.get("OPENAI_API_KEY") == "sk-FAKE-TEST-VALUE-NOT-A-CREDENTIAL"
    ...
    blob = json.dumps(st)
    assert "sk-FAKE-TEST-VALUE-NOT-A-CREDENTIAL" not in blob


def test_nexus_prefix_alias(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("GITHUB_TOKEN", raising=False)
    monkeypatch.setenv(
        "NEXUS_GITHUB_TOKEN", "ghp_FAKE_TEST_VALUE_NOT_A_CREDENTIAL"
    )
    v = Vault(workdir=tmp_path)
...
def test_redact_values_and_key_patterns(tmp_path: Path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    secret = "sk-FAKE-TEST-VALUE-NOT-A-CREDENTIAL"
...
def test_mask_mapping(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-FAKE-MASK-VALUE-NOT-A-CREDENTIAL")
    v = Vault(workdir=tmp_path)
    masked = v.mask_mapping(
        {
            "OPENAI_API_KEY": "sk-FAKE-MASK-VALUE-NOT-A-CREDENTIAL",
            "note": "hello sk-FAKE-MASK-VALUE-NOT-A-CREDENTIAL world",
            "nested": {"token": "should-mask-by-key-name"},
            "safe": 42,
        }
    )
    assert masked["OPENAI_API_KEY"] == REDACTED
    assert "sk-FAKE-MASK-VALUE-NOT-A-CREDENTIAL" not in masked["note"]
...
def test_module_helpers(tmp_path: Path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("XAI_API_KEY", "xai-FAKE-TEST-VALUE-NOT-A-CREDENTIAL")
    ...
    assert "xai-FAKE-TEST-VALUE-NOT-A-CREDENTIAL" not in vmod.redact(
        "leak xai-FAKE-TEST-VALUE-NOT-A-CREDENTIAL here", tmp_path
    )
