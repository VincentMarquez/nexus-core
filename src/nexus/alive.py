...
class AliveConfig:
    ...
    # Keep repository mutation opt-in; accepted candidates remain reviewable.
    skill_factory_auto_activate: bool = False
    ...
    def from_dict(cls, d: dict[str, Any]) -> "AliveConfig":
        ...
        return cls(
            ...
            skill_factory_auto_activate=bool(
                d.get("skill_factory_auto_activate", False)
            ),
...
def cycle_once(
    ...
    *,
    ...
    root = _root(workdir)
    cycle_baseline, baseline_ok = pub.status_porcelain_checked(root)
    cycle_baseline_status: Optional[list[str]] = (
        cycle_baseline if baseline_ok else None
    )
    ...
    if not checks.get("ok"):
        ...
        report["steps"].append({
            "step": "implement",
            "skipped": (
                "apply/self_approve not both true — no implementation apply "
                "(research, engine, candidate state, and planning docs may still update)"
            ),
            "alias": "self_approve_apply",
        })
    ...
    try:
        ...
        if plan_src.is_file():
            ...
            dest.write_text(
                ...
                "Generated by `nexus alive`. Inspect and redact before "
    ...
    if bool(getattr(cfg, "skill_factory_enable", True)):
        try:
            ...
            auto_act = bool(getattr(cfg, "skill_factory_auto_activate", False))
        ...
        if not checks.get("ok"):
            ...
            try:
                ...
                pub_res = pub.commit_and_maybe_push(
                    ...
                    baseline_status=cycle_baseline_status,
