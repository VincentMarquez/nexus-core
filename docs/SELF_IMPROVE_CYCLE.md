...
_Generated 2026-07-15 20:43 UTC_
...
- [x] From a graded mine_eval digest on disk, system loads score/idea/skill without network. (`grade_artifact`)
- [x] Workflow can be killed and resumed with correct next action. (`next_agent` checkpoint)
- [x] MCP client can query grade + run status. (4 MCP tools)
- [x] Audit log explains accept/reject with evidence path.
- [x] Demo script produces a one-screen proof for humans. (`nexus demo grade-loop`)
- [x] CI subset above is green.
