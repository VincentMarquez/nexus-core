...
## Landed this session (2026-07-15, Grok 4.5 CLI) — First apply slice §5

| P0 item | Module | Status |
|---------|--------|--------|
| **P0.1** Immutable decision ledger | `src/nexus/decision_ledger.py` | **done** — SQLite `agent_decisions`, content_hash idempotent append, tail/list_run |
| **P0.2** Action-order scheduler | `src/nexus/stages.py` | **done** — `DEFAULT_STAGES` + `SMOKE_STAGES` (`mine→grade→claim_verify`); out-of-order refused |
| **P0.3** Mine grade loader | `src/nexus/load_mine_eval.py` | **done** — fixture + IMPROVE_OURS digests (offline) |
| **P0.4** Claim verification | `src/nexus/claim_verify.py` | **done** — requires score/idea/skill/path |
| **P0.6** Smoke + tests | `src/nexus/improve_smoke.py`, CLI, tests | **done** — `nexus improve smoke\|ledger` |
| **P0.5** Worktree apply | — | **next** (explicit non-goal this PR) |

**Acceptance met:** dual append idempotent by hash · stages refuse skip · fake grade fails claim_verify · `wshobson/agents` (16.0) fixture E2E green · ledger queryable.

**Commands:**
```bash
PYTHONPATH=src python3 -m nexus.cli improve smoke --fixture tests/fixtures/mine_eval_sample.json
PYTHONPATH=src python3 -m nexus.cli improve ledger --limit 10
pytest tests/test_ledger.py tests/test_stage_order.py tests/test_claim_verify.py tests/test_improve_smoke.py -q
```

**Next PR:** worktree-isolated apply of one ported pattern (Markdown skill SoT validator from wshobson/agents) — still ledgered + claim-verified + ordered.
...
> **Status: LANDED 2026-07-15** (this session). Paths under `src/nexus/` (flat modules; no `memory/` package conflict with `memory.py`).
...
```
src/nexus/decision_ledger.py   # P0.1 agent_decisions SQLite ledger
src/nexus/stages.py            # P0.2 action-order StageRunner
src/nexus/load_mine_eval.py    # P0.3 offline grade loader
src/nexus/claim_verify.py      # P0.4 Thucy-style claim gate
src/nexus/improve_smoke.py     # smoke runner + __main__
src/nexus/cli.py               # nexus improve smoke|ledger
tests/test_ledger.py
tests/test_stage_order.py
tests/test_claim_verify.py
tests/test_improve_smoke.py
tests/fixtures/mine_eval_sample.json  # wshobson/agents 16.0 + cas 15.0
```

### Acceptance criteria

- ✅ Appending the same decision twice is idempotent by `content_hash` (lumen).  
...
- ✅ Ledger rows queryable for demo (`nexus improve ledger` / `tail()`).
...
# or
PYTHONPATH=src python3 -m nexus.improve_smoke --fixture tests/fixtures/mine_eval_sample.json
PYTHONPATH=src python3 -m nexus.cli improve smoke --fixture tests/fixtures/mine_eval_sample.json
```
