...
### Landed (2026-07-16 — First apply slice hard-apply)

| Criterion | Status | Evidence |
|-----------|--------|----------|
| Schema `work_ledger` + `grade_records` | ✅ | `src/nexus/improve_spine.py` (`nexus.improve_spine/v1`) under `.nexus_workspaces/improve_spine/` |
| Offline mine_eval ingest | ✅ | `ingest_mine_eval` / `nexus improve ingest` (fixture `tests/fixtures/mine_eval_sample.json`, cas/wshobson) |
| Stage machine + checkpoint resume | ✅ | `scouted → graded → apply_pending`; JSON checkpoint; resume skips re-grade |
| MCP `ledger.append` / `ledger.list` / `grade.get` | ✅ | tools `ledger_append`, `ledger_list`, `grade_get` in `mcp_server.py` |
| CLI `nexus improve status --run` | ✅ | prints stage, last grade, ledger events |
| Tests green | ✅ | `tests/test_improve_spine.py` + full suite **485 passed** |

```bash
PYTHONPATH=src python3 -m pytest tests/test_improve_spine.py -q
PYTHONPATH=src python3 -c "from nexus.cli import main; main(['improve','ingest','--run-id','demo-cas','--repo','codingagentsystem/cas'])"
